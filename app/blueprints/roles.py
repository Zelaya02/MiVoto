...
from app.models import Rol, MODULOS
...
def admin_required(f):
    ...
    def decorated(*args, **kwargs):
        if not current_user.tiene_permiso('roles'):
...
def index():
    roles = Rol.query.all()
    return render_template('roles/index.html', roles=roles, modulos=MODULOS)
...
def nuevo():
    if request.method == 'POST':
        ...
        if not nombre:
            ...
            return render_template('roles/form.html', rol=None, modulos=MODULOS)
...
def editar(id):
    ...
    if request.method == 'POST':
        ...
        if not nuevo_nombre:
            flash('El campo Nombre es obligatorio.', 'danger')
            return render_template('roles/form.html', rol=rol, modulos=MODULOS)
